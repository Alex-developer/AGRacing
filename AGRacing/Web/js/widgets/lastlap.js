var AGRacingLASTLAPWidget = function () {
    'use strict';
        ...
        if (_initialised) {
            for (var i = 0; i < data.PlayerStates.length; i++) {
                if (data.PlayerStates[i].IsMe === true) {
                    if (_lastLastLap !== data.PlayerStates[i].LastLapTime) {
                        if (data.PlayerStates[i].LastLapTime !== -1) {
                            jQuery('#' + _elId).html(data.PlayerStates[i].LastLapTime.toHHMMSS(false));
                        } else {
                            jQuery('#' + _elId).html('--:--:--');
                        }
                        _lastLastLap = data.PlayerStates[i].LastLapTime;
                    }
                    break;
                }
            }
...
//# sourceURL=/js/widgets/lastlap.js