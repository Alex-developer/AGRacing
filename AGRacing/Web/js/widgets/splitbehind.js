var AGRacingSPLITBEHINDWidget = function () {
    'use strict';

    var _name = 'Split Behind';
    var _icon = '/images/widgets/stopwatch.png';
    var _labels = ['Split Behind'];

    var _initialised = false;
    var _el = null;
    var _elId = null;
    var _lastSplitBehind = null;
    ...
    function updateUI(data) {
        if (_initialised) {
            if (_lastSplitBehind !== data.SplitBehind) {
                if (data.SplitBehind !== -1) {
                    jQuery('#' + _elId).html(data.SplitBehind.toHHMMSS(true));
                } else {
                    jQuery('#' + _elId).html('--:--:--');
                }
                _lastSplitBehind = data.SplitBehind;
            }
        }
    }
...
//# sourceURL=/js/widgets/splitbehind.js