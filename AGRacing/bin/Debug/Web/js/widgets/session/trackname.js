var AGRacingTRACKNAMEWidget = function () {
    'use strict';

    var _name = 'Track Name';
    var _icon = '/images/widgets/text.png';
    var _labels = ['Track', 'Track Name'];
    ...
    function buildUI() {
        ...
        var element = jQuery('<span>').css({ 'pointer-events': 'none' }).html('Track Name').attr('id', _elId);
    ...
    function updateUI(data) {
        if (_initialised) {
            if (_lastTrackName !== data.TrackName) {
                jQuery('#' + _elId).html(data.TrackName);
                _lastTrackName = data.TrackName;
            }
        }
    }
...
//# sourceURL=/js/widgets/track/trackname.js